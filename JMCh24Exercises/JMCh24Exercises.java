...
// if(birthdays==null||birthdays.length<3)return false;

// HashMap<Date,Integer>a=new HashMap<>();for(Date i:birthdays)

// {if(a.get(i)==null){a.put(i,1);}else{a.put(i,a.get(i)+1);if(a.get(i)>=3){return
// true;}}}
...
public class JMCh24Exercises
{
    ...
    public boolean has3OnSameDay(Date[] birthdays)
    {
        HashMap<Date, Integer> theMap = new HashMap<>(birthdays.length);
            ...
            if (response.length() > 0)
            {
                ...
                switch (response.charAt(0))
                {
                    case '1':
                        @SuppressWarnings("deprecation")
                        Date[] d =
                            { new Date(2010, 7, 4), new Date(2010, 1, 1), new Date(2010, 7, 4),
                    ...
                    case '3':
                        Scanner scan =
                            new Scanner("word newt word spam word word a for stuff stuff");
