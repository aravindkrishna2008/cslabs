    ...
    extends KnightCritter
{
     ...
     * with the lowest accessibility. It will return the location with the
     * lowest accessibility. In the case of a tie, it returns the first Location
     * within the ArrayList with the lowest value. If all accessibility values
     * are < 0, the method returns null
     * 
     * @param locs
    ...
    public Location getBestMove(ArrayList<Location> locs)
    {
        ...
        {
            if (accessGrid[locs.get(i).getRow()][locs.get(i).getCol()] < minm)
            {
                minm = accessGrid[locs.get(i).getRow()][locs.get(i).getCol()];
                bestMove = locs.get(i);
            }
        }
     ...
     * to go negative. It will also change its own accessibility value to a
     * negative number.
     * 
     * @param loc
    ...
    public void updateAccess(Location loc)
    {
        ...
        if (accessGrid[row][col] >= 0)
        {
            accessGrid[row][col] = -1;
        }

        for (int i = 0; i < getPossibleMoves(loc).size(); i++)
        {
            int r = getPossibleMoves(loc).get(i).getRow();
            int c = getPossibleMoves(loc).get(i).getCol();
