...
public class Quilt
{
    ...
    {
        char[][] a = new char[myRowsOfBlocks * myBlock.length][myColsOfBlocks * myBlock[0].length];
        for (int r = 0; r < myRowsOfBlocks; r++)
        {

            for (int c = 0; c < myColsOfBlocks; c++)
            {
                int row = r * myBlock.length;
                int col = c * myBlock[0].length;
                if ((r + c) % 2 == 0)
                {
                    placeBlock(row, col, a);
                }
                else if ((r + c) % 2 == 1)
                {
                    placeFlipped(row, col, a);
                }
            }
        }
        return a;
