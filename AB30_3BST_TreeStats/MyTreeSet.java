...
public class MyTreeSet<E>
{
    ...
    private int width(TreeNode<E> root)
    {
        ...
        int max2 = left + right + 1;

        int leftWidth = width(root.getLeft());
        int rightWidth = width(root.getRight());

        return max(max2, Math.max(leftWidth, rightWidth));
    }
    ...
    private void printLevel(TreeNode<E> root, int level)
    {
        ...
        if (root == null || level >= height(root))
        {
            return;
        }
        else if (level == 0)
    ...
    private TreeNode<E> findPtr(TreeNode<E> root, Comparable target)
    {
        ...
        findPtr(root.getLeft(), target);
        findPtr(root.getRight(), target);
        return null;
